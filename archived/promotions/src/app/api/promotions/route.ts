...
export async function POST(req: Request) {
    try {
        ...
        if (targetIds.length === 0) {
            return NextResponse.json({ error: "At least one product identifier is required to link promotion registry." }, { status: 400 })
        }

        const promoPayload = {
            productId: String(targetIds[0]), // Legacy support for main column
            title: title || name || "UNNAMED PROTOCOL",
        ...
        return NextResponse.json({ 
            ...
            details: error.message || "Unknown synchronization error",
            code: error.code
        }, { status: 500 })
    }
}
...
export async function GET(req: Request) {
    try {
        ...
        const enrichedPromotions = (promotions || []).map((promo: any) => {
            ...
            return {
                ...promo,
                name: promo.name || promo.title || "UNNAMED PROTOCOL",
